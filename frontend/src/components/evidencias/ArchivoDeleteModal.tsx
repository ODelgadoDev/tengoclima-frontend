...
interface ArchivoDeleteModalProps {
  archivo: ArchivoTrabajo;
  onClose: () => void;
  onDeleted: () => void;
}

export function ArchivoDeleteModal({
  archivo,
  onClose,
  onDeleted,
}: ArchivoDeleteModalProps) {
  ...
  const handleDelete = async () => {
    ...
    try {
      await evidenciasApi.deleteArchivo(archivo.id);
      onDeleted();
    } catch (error) {
      setErrorMessage(
        getApiErrorMessage(
          error,
          "No fue posible enviar el archivo a la papelera.",
        ),
      );
    } finally {
      setIsDeleting(false);
    ...
    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-slate-950/55 p-4">
      <div className="w-full max-w-md rounded-2xl bg-white p-5 shadow-2xl">
        ...
        <p className="mt-2 text-sm leading-6 text-slate-500">
          <strong>{archivo.nombre_original}</strong> dejará de aparecer, pero
